        ...
        <div className="mt-12 border-t border-indigo-800 pt-8">
          <div className="md:flex md:items-center md:justify-between">
            <div className="flex space-x-6 md:order-2">
              ...
              <FooterLink label="Cookie Policy" href="#" />
            </div>
            <p className="mt-8 text-base text-indigo-200 md:mt-0 md:order-1">
              © 2025 Modern CPA. All rights reserved.
            </p>
          </div>
        </div>
      </div>
...
function FooterSection({ title, links }) {
  return (
    <div>
      <h3 className="text-sm font-semibold text-indigo-200 tracking-wider uppercase">{title}</h3>
      <ul className="mt-4 space-y-2">
        {links.map((link) => (
          <li key={link.label}>
            <a 
              href={link.href} 
              className="text-base text-indigo-300 hover:text-white transition-colors duration-200 flex items-center"
            >
              {link.icon}
              {link.label}
            </a>
